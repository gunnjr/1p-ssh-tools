...
if ((NO_AGENT)); then
  echo
  echo "(Skipping agent.toml changes due to --no-agent)"
  exit 0
fi
...
echo
echo ">>> 1Password SSH Agent policy"
...
if ((do_write)); then
  ...
  echo
  echo "agent.toml updated. Backup: ${AGENT_FILE}.bak-${stamp}"
